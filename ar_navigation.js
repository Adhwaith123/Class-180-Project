    ...
    $.ajax({
        ...
        success:function(response){
            let images={
                ...
                "left_right":"ar_left.png",
                "slight_right":"ar_slight_right.png",
                "slight_left":"ar_slight_left.png",
                "straight":"ar_straight.png",
                "start":"ar_start.png",
            }
            let steps=response.roots[0].legs[0].steps
            for(let i=0; i<steps.length;i++){
                let image
                let distance=steps[i].distance
                let instruction=steps[5].maneuver.instruction
                if(instruction.includes("Turn right")){
                    image="turn_right"
                }
                else if(instruction.inclued("Turn left")){
                    image="turn_left"
                }
                if(i>0){
                    $("#scene_container").append(
                        <a_entity gps_entity_place="latitude:$(steps[i];maneuver.location([1];
                        longtitude:$(steps[1].maneuver.location[0])))">
